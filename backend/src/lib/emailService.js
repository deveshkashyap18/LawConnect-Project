...
export const sendOtpEmail = async (toEmail, otp) => {
  ...
  // PROXY APPROACH FOR RENDER PRODUCTION
  if (process.env.NODE_ENV === "production") {
    try {
      console.log(`Sending OTP via Vercel Proxy to ${toEmail}...`);
      const response = await fetch("https://law-connect-project.vercel.app/api/send-email", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          html: htmlContent,
          secret: process.env.JWT_SECRET || "lawconnect_super_secret_jwt_key_2024"
        })
      });
      ...
      console.log(`📨 Proxy OTP sent to ${toEmail} | MessageID: ${data.messageId}`);
      return true;
    } catch (error) {
  ...
  // DIRECT APPROACH FOR LOCAL DEVELOPMENT
  ...
  try {
    ...
    console.log(`📨 Local OTP sent to ${toEmail} | MessageID: ${info.messageId}`);
    return true;
  } catch (error) {
    console.error("❌ Failed to send local email:", error);
    return false;
  }
};
