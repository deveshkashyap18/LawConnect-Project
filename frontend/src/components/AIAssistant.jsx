...
import { Bot, X, Send, Minus, Square, Minimize2, Maximize2, Trash2, History, MessageSquarePlus, Pencil } from "lucide-react";
            ...
            <>
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-muted/20">
                {messages.map((msg, idx) => (
                  <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end group' : 'justify-start'}`}>
                    {msg.role === 'user' && (
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-6 w-6 mr-2 opacity-0 group-hover:opacity-100 transition-opacity self-center text-muted-foreground hover:text-foreground hover:bg-muted"
                        onClick={() => {
                    ...
                    <div className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm whitespace-pre-wrap ${msg.role === 'user' ? 'bg-primary text-white rounded-tr-none' : 'bg-muted border text-foreground rounded-tl-none'}`}>
                      {msg.content}
                    </div>
                  </div>
                ))}
